...
N3Store.prototype = {
  ...
  addTriple: function (subject, predicate, object, graph) {
   ...
   if(N3Util.isLiteral(object) && N3Util.getLiteralType(object) == '<http://www.opengis.net/ont/geosparql#wktLiteral>') {
  ...
  findByIRI: function (subject, predicate, object, graph) {
    ...
    if(N3Util.isLiteral(object)) {
      console.log(N3Util.getLiteralType(object));
      // && N3Util.getLiteralType(object) == '<http://www.opengis.net/ont/geosparql#wktLiteral>') {
      ...
      console.log("quadStore query :" + geoms.length);
      for (var i = geoms.length - 1; i >= 0; i--)
      {
        ...
        if (subject) {
            ...
            results.push( this._findInIndex(graphItem.objects, geoms[i], subject, predicate,
                                     'object', 'subject', 'predicate', graph));         
        }
        else if (predicate)
          // If only predicate and possibly object are given, the predicate index will be the fastest.
          results.push( this._findInIndex(graphItem.predicates, predicate, geoms[i], null,
                                   'predicate', 'object', 'subject', graph));
        else
          // If only object is given, the object index will be the fastest.
          results.push( this._findInIndex(graphItem.objects, geoms[i], null, null,
                                   'object', 'subject', 'predicate', graph));
      }
      console.log("spatial found " + results);
      return results;
    }
