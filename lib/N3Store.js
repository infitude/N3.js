...
function N3Store(triples, options) {
  ...
  this._quadTree = rbush();
  // set the minExtent to a reasonable lat/long value for a point (default is 1.0 ie a degree!)
  //this._quadTree.minExtent = 0.0001;
...
N3Store.prototype = {
  ...
  addTriple: function (subject, predicate, object, graph) {
   ...
   // Keep a spatial literal for later in this function...
   var geoSpatialObject;
   if(N3Util.isLiteral(object) && (N3Util.getLiteralType(object) === '<http://www.opengis.net/ont/geosparql#wktLiteral>' || N3Util.getLiteralType(object) === 'http://www.opengis.net/ont/geosparql#wktLiteral')) {
      geoSpatialObject = object;      
    }
    ...
    // Add the envelope of the spatial literal and object # to the spatial quadtree index
    if(geoSpatialObject !== undefined) {
      var wkt = N3Util.getLiteralValue(geoSpatialObject);
      var reader = new jsts.io.WKTReader();
      var geom = reader.read(wkt);
      var env = geom.getEnvelopeInternal();
      console.log('feature env :' + env);
      var item = [env.minx, env.miny, env.maxx, env.maxy, {"id": object}];
      //this._quadTree.insert(env, object);
  ...
  findByIRI: function (subject, predicate, object, graph) {
    ...
    // before translating the object IRI, check if its spatial and 
    // if so query the spatial index to get intersecting internal index keys
    if(N3Util.isLiteral(object) && (N3Util.getLiteralType(object) === '<http://www.opengis.net/ont/geosparql#wktLiteral>' || N3Util.getLiteralType(object) === 'http://www.opengis.net/ont/geosparql#wktLiteral')) {
      var wkt = N3Util.getLiteralValue(object);
      var reader = new jsts.io.WKTReader();
      var geom = reader.read(wkt);
      var env = geom.getEnvelopeInternal();
      console.log('search env: ' + env);
      var results = [];
     // var geoms = this._quadTree.queryByEnvelope(env);
      var item = [env.minx, env.miny, env.maxx, env.maxy];      
      var geoms = this._quadTree.search(item);
      for (var i = geoms.length - 1; i >= 0; i--)
      {
        // Choose the optimal index, based on what fields are present (always have an object)
        if (subject) {
            // If subject and object (always for spatial) are given, the object index will be the fastest.
            results = results.concat( this._findInIndex(graphItem.objects, geoms[i][4].id, subject, predicate,
                                     'object', 'subject', 'predicate', graph));         
        }
        else if (predicate)
          // If only predicate and possibly object are given, the predicate index will be the fastest.
          results = results.concat( this._findInIndex(graphItem.predicates, predicate, geoms[i][4].id, null,
                                   'predicate', 'object', 'subject', graph));
        else
          // If only object is given, the object index will be the fastest.
          results = results.concat( this._findInIndex(graphItem.objects, geoms[i][4].id, null, null,
                                   'object', 'subject', 'predicate', graph));
      }
      return results;
    }
